...
function renderState(state) {
	...
	if (state.in) document.body.classList.add('in')
...
function updateWithAnimation(nextValSelector, name) {
	...
	if (currentVal == nextValSelector()) return
	...
	setTimeout(() => {
		document.querySelector('#' + name + ' .content').innerText = document.querySelector(
			'#' + name + '-next .content'
		).innerText
...
function getDownsAndGains(state) {
	...
	if (downs == '') return gains
	if (gains == '') return downs
	return downs + ' & ' + gains
}
...
function pad(val, len) {
	val = val + ''
	if (val.length < len) return pad('0' + val, len)
