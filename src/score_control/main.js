...
if (window.location.hash.indexOf('simple') >= 0) {
	...
	document.querySelector('#bar-preview').src = '/score_simple.html'
}

window.CurrentState = {}
window.NextState = {}

const init = { method: 'GET', mode: 'cors', headers: { Accept: 'application/json' }, cache: 'default' }
...
function renderQuarter(state, next) {
	...
	if (next.quarter !== undefined) elm.classList.add('pending')
	...
	if (opt !== null) opt.classList.add('current')

	const opt2 = document.querySelector('#quarter-select option[value="' + next.quarter + '"]')
	if (next.quarter !== undefined && opt2 !== null) opt2.selected = true
	else if (opt !== null) opt.selected = true
}
...
function renderFlag(state, next) {
	...
	if (next.flag !== undefined) elm.classList.add('next')
}
...
function renderInOut(state, next) {
	...
	if (next.in !== undefined) elm.classList.add('next')
	else if (state.in) elm.classList.add('on')
	else elm.classList.add('off')
}
...
function renderButtonGroup(state, next, key) {
	...
	if (elm !== null) elm.classList.add('current')
	if (next[key] !== undefined) {
		const elm2 = document.querySelector('.btn[data-key="' + key + '"][data-value="' + next[key] + '"]')
		if (elm2 !== null) elm2.classList.add('pending')
	}
}
...
document.querySelector('#gains-slider').oninput = () => {
	...
	if (window.NextState.gains == window.CurrentState.gains) window.NextState.gains = undefined
...
document.querySelectorAll('.btn-editor').forEach((e) => {
	...
	e.onclick = () => {
		const newVal = window.prompt(
			key,
			window.NextState[key] === undefined ? window.CurrentState[key] : window.NextState[key]
		)
...
function buttonGroupHandler(e) {
	...
	let next = current
	if (e.hasAttribute('data-delta')) next = parseInt(next) + parseInt(e.getAttribute('data-delta'))
	if (e.hasAttribute('data-value')) next = e.getAttribute('data-value')

	if (next == window.CurrentState[key] || clear) window.NextState[key] = undefined
	else window.NextState[key] = next
...
document.querySelector('#quarter-select').onchange = (e) => {
	...
	if (newVal == window.CurrentState.quarter) window.NextState.quarter = undefined
	else window.NextState.quarter = newVal
	...
	elm.onclick = (e) => {
		...
		if (e2.hasAttribute('data-touchdown')) {
			if (e2.hasAttribute('data-clear')) {
				if (window.NextState.touchdown == e2.getAttribute('data-touchdown')) window.NextState.touchdown = undefined
			} else {
