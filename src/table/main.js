...
window.next = function () {
	if (window.currentData + 1 >= window.tableData.length) return window.stop()
...
function showTableData(i) {
	...
	if (newData.mode == 'leader') leaderHeader.classList.add('show')

	document.querySelector('#line2 .left').innerText =
		newData.subtitle == '' || newData.subtitle === undefined || newData.subtitle === null ? ' ' : newData.subtitle
